 ...
 * @returns Node
 */
export async function handleJavascriptEntryNode(node: Promise<Node>, graph: Graph) {
 ...
 */
export async function getJavascriptNodeMeta(nodePath: any, graph: Graph, internalPath?: InternalPath): Promise<Node> {
 ...
 * @returns Function
 */
export function getJavascriptNodesFromHtmlNode(htmlNode: Node, graph: Graph): Function {
  const walkToScriptTag = (tree: any, htmlNode: any, graph: Graph): void => {
    tree.forEach(
      (node: any) => {
        if (node.name === 'script') {
          const isFirstChildText = node && node.content && node.content[0] && node.content[0].type === "text";
          const hasOnlyChildText = isFirstChildText && node.content.length === 1;
          const hasAttrs = node !== undefined
            && node.attrs !== undefined
            && node.attrs.src !== undefined
            && node.attrs.src[0] !== undefined

          if (hasAttrs) {
            if (node.attrs.src[0].type === "text" && !node.attrs.src[0].content.startsWith("http")) {
          ...
          if (hasOnlyChildText) {
            const location: InternalPath = getInternalNodeLocation(node.content[0], true);
