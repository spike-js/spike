...
var require_utils = __commonJS((exports2) => {
  ...
  exports2.help = function(bin, tree, key, single) {
  ...
  exports2.error = function(bin, str, num = 1) {
  ...
  exports2.sentences = function(str) {
...
var require_main = __commonJS((exports2, module2) => {
    ...
    if (typeof module22 === "object" || typeof module22 === "function") {
      for (let key in module22)
        if (!__hasOwnProperty2.call(target, key) && key !== "default")
          __defineProperty2(target, key, {get: () => module22[key], enumerable: true});
    }
    return target;
  };
  var __toModule2 = (module22) => {
    if (module22 && module22.__esModule)
      return module22;
    return __exportStar2(__defineProperty2({}, "default", {value: module22, enumerable: true}), module22);
  };
  ...
  function createChannel(options) {
    ...
    return {
      ...
      service: {
        ...
        transform(input, options2, isTTY2, fs22, callback) {
...
${file}:${line}:${column}: error: ${e.text}`;
  ...
  const path = __toModule2(require("path"));
...
function addBuildCommand(prog, options) {
  prog.command("build <...entryPoints>", "TODO", {default: true}).describe("Build your Spike project once").action(buildCommandHandler(options));
}
function buildCommandHandler(options) {
  return (entryPoints) => __async(this, [], function* () {
    ...
    try {
      ...
      const results = yield Promise.all(outputBundles.map((bundle) => __async(this, [], function* () {
        const bundlePath = bundle.file.replace(options.workingDirectory, options.outputDirectory).replace(".ts", ".js");
        const bundleMapPath = bundle.file.replace(options.workingDirectory, options.outputDirectory).replace(".ts", ".map.js");
...
// src/cli.ts
__export(exports, {
  createCommandLine: () => createCommandLine,
  default: () => cli_default
});
const sade = __toModule(require_lib2());
const CLI = sade.default("spike-scripts");
function createCommandLine(options) {
  return __async(this, [], function* () {
    build_default(CLI, options);
    return (argv) => CLI.parse(argv);
  });
}
var cli_default = createCommandLine;
